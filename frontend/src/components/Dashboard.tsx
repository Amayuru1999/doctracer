      ...
      {(comparison.amendment_gazette.structure as any).note && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <div className="flex items-start">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-yellow-800">Limited Change Tracking</h3>
              <div className="mt-2 text-sm text-yellow-700">
                <p>{(comparison.amendment_gazette.structure as any).note}</p>
                <p className="mt-1">The comparison below shows the base gazette structure. Detailed change tracking for this amendment is not available in the current dataset.</p>
              </div>
            </div>
          </div>
        </div>
            ...
            >
              ...
              {amendments.map(a => (
                <option key={a.gazette_id} value={a.gazette_id}>
                  {a.gazette_id} — {a.published_date} {!(a as any).has_detailed_changes ? '(Limited tracking)' : ''}
                </option>
              ))}
            </select>
          </div>
