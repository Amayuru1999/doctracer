...
export async function getGazetteDetails(
  gazetteId: string
): Promise<GazetteDetail[]> {
...
export async function getGazetteFullDetails(
  gazetteId: string
): Promise<GazetteFullDetails> {
  const r = await fetch(
    `${API_URL}/gazettes/${encodeURIComponent(gazetteId)}/details`
  );
...
export async function getGazetteStructure(
  gazetteId: string
): Promise<GazetteStructure> {
  const r = await fetch(
    `${API_URL}/gazettes/${encodeURIComponent(gazetteId)}/structure`
  );
...
export async function getGovernmentEvolutionFromBase(
  baseId: string
): Promise<GovernmentEvolutionFromBase> {
  const r = await fetch(
    `${API_URL}/network/government-evolution/${encodeURIComponent(baseId)}`
  );
  if (!r.ok) throw new Error(`/network/government-evolution/:id ${r.status}`);
  return r.json();
}

export async function compareGazetteStructures(
  baseId: string,
  amendmentId: string
): Promise<GazetteComparison> {
  const r = await fetch(
    `${API_URL}/gazettes/${encodeURIComponent(
      baseId
    )}/compare/${encodeURIComponent(amendmentId)}`
  );
